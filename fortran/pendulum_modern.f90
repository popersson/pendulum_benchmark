...
contains
  ...
  pure subroutine runge5(f, y0, h, nsteps, y)
    ...
    real(dp) :: k1(neq), k2(neq), k3(neq), k4(neq), k5(neq), k6(neq)
    integer :: n
    ...
    do n = 1, nsteps
       associate (yn => y(:,n))
         call f(yn, k1)
         call f(yn + h*k1/5, k2)
         call f(yn + h*2*k2/5, k3)
         call f(yn + h*9*k1/4 - h*5*k2 + h*15*k3/4, k4)
         call f(yn - h*63*k1/100 + h*9*k2/5 - h*13*k3/20 + h*2*k4/25, k5)
         call f(yn - h*6*k1/25 + h*4*k2/5 + h*2*k3/15 + h*8*k4/75, k6)
         y(:,n+1) = yn + h*(17*k1 + 100*k3 + 2*k4 - 50*k5 + 75*k6) / 144
       end associate
    end do
  end subroutine runge5

end module pendulum_model


program pendulum
  use iso_fortran_env, only: int64
  use pendulum_model, only: dp, neq, fpend, runge5
  implicit none (type, external)
